...
Turn on **Quick quit** beside apps you are happy to close, then use **Free up memory** to request normal Quit for all running marked apps (independent of table filters). Choices are remembered by app path in this browser's local storage, including apps that are no longer running. Only running, quittable apps show chips; remove a running app using its × or table toggle. Nothing quits automatically, and protected apps are always skipped. The action refreshes process identities first and never escalates to Force Quit. Reported memory is an estimate, not a promise of RAM recovered.

The **CPU overview** shows system-wide busy/idle CPU and up to five minutes of recent history. Each refresh measures CPU counters over a fresh one-second interval. App CPU sums helpers; 100% means one busy logical core, so multicore apps can exceed 100%. Total CPU is normalized across all logical cores using [Node’s OS CPU counters](https://nodejs.org/api/os.html#oscpus). Newly started processes show “Sampling…” until an interval is available. **Top CPU users** ranks the five busiest app/developer groups regardless of table filters, with individual normal Quit buttons and protected processes disabled. History accumulates while requested, stays in server memory, and resets on server restart. Automatic refresh pauses while interacting with the process table or CPU list to keep action targets stable. System services count toward total CPU but are not listed as quittable apps.
